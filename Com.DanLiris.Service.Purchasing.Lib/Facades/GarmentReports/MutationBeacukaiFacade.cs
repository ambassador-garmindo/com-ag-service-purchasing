...
namespace Com.DanLiris.Service.Purchasing.Lib.Facades.GarmentReports
{
    public class MutationBeacukaiFacade : IMutationBeacukaiFacade
    {
        ...
        public List<MutationBBCentralViewModel> GetCentralItemBBReport(DateTime? datefrom, DateTime? dateto, int offset)
        {
            ...
            var ReceiptBalance = (from a in (from aa in dbContext.GarmentUnitReceiptNotes
                                             where aa.CreatedUtc.Date < DateFrom.Date && aa.UId == null && aa.IsDeleted == false && pemasukan.Contains(aa.URNType) && unit.Contains(aa.UnitCode) && storage.Contains(aa.StorageName)
                                             select aa)
                                  join b in (from bb in (from bbb in dbContext.GarmentUnitReceiptNoteItems where categories1.Contains(bbb.ProductName) && bbb.IsDeleted == false select bbb) where customs.Contains(bb.CustomsCategory) select bb) on a.Id equals b.URNId
            ...
            var Receipt = (from a in (from aa in dbContext.GarmentUnitReceiptNotes where aa.CreatedUtc.Date >= DateFrom.Date && aa.CreatedUtc.Date <= DateTo.Date 
                                      && aa.UId == null && aa.IsDeleted == false && pemasukan.Contains(aa.URNType) && unit.Contains(aa.UnitCode) && storage.Contains(aa.StorageName)
                                      select aa)
                           join b in (from bb in (from bbb in dbContext.GarmentUnitReceiptNoteItems where categories1.Contains(bbb.ProductName) && bbb.IsDeleted == false select bbb) where customs.Contains(bb.CustomsCategory) select bb) on a.Id equals b.URNId
                           join c in dbContext.GarmentExternalPurchaseOrderItems.IgnoreQueryFilters() on b.EPOItemId equals c.Id
                           join d in dbContext.GarmentExternalPurchaseOrders.IgnoreQueryFilters() on c.GarmentEPOId equals d.Id
            ...
            var dataUnion = ReceiptBalance.Union(ExpenditureBalance).Union(Receipt).Union(Expenditure).AsEnumerable();
              
            #endregion

            var mutation = dataUnion.GroupBy(x=>new { x.ItemCode, x.ItemName, x.UnitQtyName, x.SupplierType, x.Gudang },(key,group) => new MutationBBCentralViewModel
            {
