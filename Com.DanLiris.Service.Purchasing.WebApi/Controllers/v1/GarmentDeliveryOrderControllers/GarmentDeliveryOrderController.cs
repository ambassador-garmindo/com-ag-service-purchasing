...
namespace Com.DanLiris.Service.Purchasing.WebApi.Controllers.v1.GarmentDeliveryOrderControllers
{
    ...
    public class GarmentDeliveryOrderController : Controller
    {
        ...
        [HttpGet("correction-note-price")]
        public IActionResult GetForCorrectionNotePrice(int page = 1, int size = 25, string order = "{}", string keyword = null, string filter = "{}")
        {
            try
            {
                identityService.Username = User.Claims.Single(p => p.Type.Equals("username")).Value;

                var result = facade.ReadForCorrectionNotePrice(page, size, order, keyword, filter);
